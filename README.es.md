...
| "¿Mi cron diario `claude -p` me cobrará dinero?" | Con una suscripción consume de los límites de uso de tu plan — el mismo pool que el uso interactivo, sin crédito por ejecución aparte (el cambio de crédito facturado por separado anunciado para 2026-06-15 quedó en pausa y no está en vigor; las cuentas con `ANTHROPIC_API_KEY` siguen en pago por uso). Las filas de facturación contienen el estado verificado actual y su fecha. |
