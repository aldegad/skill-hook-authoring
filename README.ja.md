...
| 「毎晩の`claude -p` cronは課金されるか？」 | 課金行にはヘッドレス/SDK利用の課金ルールを正確に追記している。ここには2026-06-15のAgent SDKクレジット変更も含まれるが、現時点では**一時停止中**（2026-06-18時点で、`claude -p`は引き続きサブスクリプションの利用枠から消費される）。 |
...
（課金について：2026-06-15に発表された月額個別クレジット変更は現在**一時停止中**のため、2026-06-18時点では`claude -p`およびAgent SDKの利用は引き続きサブスクリプションの利用枠から消費される。`ANTHROPIC_API_KEY`を利用するユーザーは従量課金のまま。）詳しくは`docs/cloud-automation.md`を参照。
