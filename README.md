...
| "Will my nightly `claude -p` cron cost me money?" | The billing rows track exactly how headless/SDK usage is charged — including the 2026-06-15 Agent SDK credit change, which is currently **paused** (as of 2026-06-18, `claude -p` still draws from your subscription's usage limits). |
...
The recommended path is **Claude Routines** — a scheduled Claude Code session that runs in Anthropic's cloud on a Claude subscription, with no API key and no GitHub Actions, and keeps running when your laptop is closed. Running the refresh **locally** via `claude -p` is discouraged mainly for **reliability**: a local cron fires only while the machine is awake, while a cloud Routine runs regardless of laptop state. (On billing: the separate-monthly-credit change announced for 2026-06-15 is currently **paused**, so as of 2026-06-18 `claude -p` and Agent SDK usage still draw from your subscription's usage pool; `ANTHROPIC_API_KEY` users remain pay-as-you-go.) See `docs/cloud-automation.md`.
