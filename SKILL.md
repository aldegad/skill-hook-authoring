...
|---|---|---|---|
| Codex | Repo `.agents/skills/` plus user/admin/system skill roots | User/project `.codex/hooks.json` or inline config hooks, plus plugin-bundled hooks | Portable root `plugin.json` (Agent Plugins schema) with `skills/`, `mcp.json`, and OpenAI-specific presentation / MCP server mappings / hook settings under `extensions.com.openai`, plus marketplace metadata; `.codex-plugin/plugin.json` is still accepted as a compatibility fallback |
...
| Hermes Agent | Skills, skill taps, and plugin-bundled skills are documented | Gateway hooks, shell hooks, plugin hooks, and outbound webhooks (signed events pushed to external HTTP endpoints) are documented | `plugin.yaml` plugins can bundle tools, hooks, slash commands, CLI commands, and skills; five documented roots — `~/.hermes/plugins/`, project-local `./.hermes/plugins/` (needs `HERMES_ENABLE_PROJECT_PLUGINS=true`), bundled `<repo>/plugins/`, pip entry points under `hermes_agent.plugins`, and Nix declarative installs (`services.hermes-agent.extraPlugins` / `extraPythonPackages`). Plugin hooks register via `ctx.register_hook()`, so project-local hooks are reachable; portable Agent Plugins v1.0.0 packages (root `plugin.json` + `skills/` + `mcp.json`) install as a compatibility adapter, disabled until explicitly enabled |
...
**Explicit skill invocation is not the same token across runtimes.** Claude Code, Grok, and Cursor expose user-invocable skills as slash commands such as `/<skill-name>` (Claude/Cursor/Grok: `disable-model-invocation: true` makes a skill explicit-only); Codex uses `/skills` (selector) or `$<skill-name>` (mention) — typed `/<skill-name>` is not a documented Codex form, and `allow_implicit_invocation: false` in `agents/openai.yaml` turns off description-matching; Hermes registers every installed skill in `~/.hermes/skills/` as a typed `/<skill-name>` slash command, and Antigravity likewise registers each skill as a typed `/<skill-name>` slash command in the TUI; community gajae-code is the odd one out — it uses a **colon** form, `/skill:<name>` (e.g. `/skill:deep-interview`), not `/<skill-name>`. Full source-cited table: `docs/compatibility-matrix.md` → **Skill Invocation**. For cross-engine commands, rely on description-triggered invocation as the portable layer and treat the typed token as per-engine sugar.
...
| Antigravity CLI | `agy` | `agy -p "<p>"` (`--print`/`--prompt`; dedicated Headless-mode page) |
...
- **Antigravity CLI** (`agy`, the Gemini CLI successor) resumes with `agy -c` / `agy --continue` (most recent in the workspace) or `agy --conversation <conversation-id>` — both now also documented to work in headless mode (`agy -p … --continue`); conversations are **workspace-scoped** (it lists only sessions started in that cwd). `-c` resolves through a documented workspace-keyed cache map, `~/.gemini/antigravity-cli/cache/last_conversations.json` — the one Antigravity session store the docs state.
- **gajae-code** documents **no CLI resume flag**; session ids exist only on the SDK path (allowlisted lifecycle ops `session.create`/`session.fork`/`session.resume`/`session.close` via `gjc sdk session raw`, plus `gjc sdk session tail <sessionId> --until-idle`). It uses a project `.gjc/` dir (customization scope; SDK endpoint records under `.gjc/state/sdk`) and isolates work with git worktrees (`gjc --tmux --worktree <name>`), but neither worktree isolation nor broker-bound external control is an interactive `--resume` — treat CLI resume as `not documented`.
...
- **Claude / Claude Code** — current: Fable 5.1 (`claude-fable-5-1`, "for demanding reasoning and long-horizon agentic work" — supersedes Fable 5, 2026-09-11), Opus 5 (`claude-opus-5`, page now says "for most workloads"), Sonnet 5 (`claude-sonnet-5`), Haiku 4.5 (`claude-haiku-4-5`, id `claude-haiku-4-5-20251001`). Legacy: Fable 5 (→ 5.1, 2026-09-11), Opus 4.8 (→ 5), Opus 4.7, and Sonnet 4.6 (→ 5); `claude-opus-4-1-20250805` retired 2026-08-05 (the model-deprecations page recommends `claude-opus-4-8`). Reasoning is **adaptive thinking** on the current generation (Haiku 4.5: extended thinking); the `--effort` labels (`low`, `medium`, `high`, `xhigh`, `max`, `ultracode`) are a Claude Code caller-layer selector, not distinct vendor models. Claude Mythos (Project Glasswing) is documented but invitation-only (not GA) — do not treat it as spawnable; the current Mythos id is `claude-mythos-5-1`.
- **Codex** — current flagship: GPT-6 Astra (`gpt-6-astra`, listed first under "Recommended models" as the most capable model for complex work across code, apps, and research), above the GPT-5.6 family `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna` (bare `gpt-5.6` appears only as CLI example shorthand, not a listed id); `gpt-5.3-codex-spark` is still a text-only research preview. Under "Other models", `gpt-5.5` is the previous-generation flagship, retiring from Codex (ChatGPT sign-in) on 2026-10-14 — replace it with `gpt-5.6-sol`, and `gpt-5.4` / `gpt-5.4-mini` are still listed even though the page's stated ChatGPT-sign-in retirement date (2026-08-31) has passed — do not select them under ChatGPT sign-in (replace with `gpt-5.6-terra` / `gpt-5.6-luna`). `gpt-5.2` and `gpt-5.3-codex` are already deprecated. Reasoning effort is selected separately (Low through Ultra); `model_reasoning_effort` + `service_tier` stay config knobs.
- **Grok** — current flagship: `grok-4.6` (500k context; **configurable reasoning**), plus the Grok Voice API and Imagine image/video APIs. The Text API pricing table also lists `grok-4.5` (500k), `grok-4.3`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning`, `grok-4.20-multi-agent-0309` (1M each), and `grok-build-0.1` (256k). The Voice Pricing section names `grok-voice-think-fast-2.0` (Speech to Speech); `grok-voice-think-fast-1.0` no longer appears and its retirement is not documented. Effort is documented at **both layers**: the CLI reference lists `--effort <LEVEL>`, while the API reasoning page gives `grok-4.6` `low`/`medium`/`high`/`xhigh` (default `high`) and `grok-4.5` `low`/`medium`/`high` (default `high`); reasoning cannot be disabled. A dedicated retirement page exists (`/developers/migration/may-15-retirement`): eight ids retired 2026-05-15 with redirects (e.g. `grok-code-fast-1` → `grok-build-0.1`); see `docs/models/grok.md`.
- **Gemini-Antigravity** — provider-routed; ids verified against Google's official model page (<https://ai.google.dev/gemini-api/docs/models?hl=en>, page last-updated 2026-09-15). **`gemini-3.8-flash` is the lead Stable Flash model** ("Our most intelligent Flash model"), engineered for long-horizon software engineering, autonomous agents, and complex enterprise workflows, superseding `gemini-3.7-flash` as the lead row; the 3.8 generation now also ships two Stable **Live API** models, `gemini-3.8-live` (the documented default for low-latency voice agents) and `gemini-3.8-live-extended-thinking` (high-reasoning voice), which supersede the now-legacy `gemini-3.1-flash-live-preview`; `gemini-3.7-flash` and `gemini-3.6-flash` remain Stable as previous generations, while `gemini-3.5-flash` is still labelled legacy. Stable `gemini-3.5-flash-lite`, `gemini-3.1-flash-lite`, and preview ids (`gemini-3.1-pro-preview`, `gemini-3-flash-preview`, Antigravity Agent `antigravity-preview-05-2026`) remain. The CLI's headless-mode sample (`https://antigravity.google/docs/cli/headless`, re-verified 2026-09-18) surfaces `gemini-3.8-flash-high`, `gemini-3.8-flash-medium`, `gemini-3.7-flash-high`, `gemini-3.7-flash-medium`, `gemini-3.6-flash-high`, `gemini-3.6-flash-medium`, `gemini-3.1-pro-high`, and `claude-sonnet-4-6` — CLI-surface slugs not on the API models page.
...
- **Cursor** — mixed: it ships **Cursor-owned** models (Composer 2.5 and Cursor Grok 4.6 / 4.5 from Cursor and SpaceXAI; Composer 1 no longer appears) *and* routes to a wide third-party lineup (the Other Models table spans the full Claude line up to Fable 5.1/Opus 5, the full GPT-5 line including GPT-5.1/5.2 Codex variants and GPT-5.4 Nano, Gemini 3 Pro/3.1 Pro/3.5–3.8 Flash, GLM 5.2, Kimi K2.7 Code/K3, and a new Meta entry `Muse Spark 1.3`), plus `Auto` (not available on the Start plan). Cursor Grok 4.6 supports `low`/`medium`/`high`/`xhigh` effort and a Fast tier, though the Start plan pins Grok 4.6 / 4.5 to a fixed medium effort in non-fast mode. `Auto` is still not a pricing-table row and has three documented modes — Cost, Balance, Intelligence, each billed at the routed model's list price — with Cursor Router picking the model on Teams/Enterprise.
...
| Hermes Agent | `.hermes.md` / `HERMES.md`, then `AGENTS.override.md`, then `AGENTS.md`, then `CLAUDE.md`, then `.cursorrules` — one project context type loads per session on that first-match chain. `AGENTS.override.md` is a personal, per-directory override loaded instead of the committed `AGENTS.md` next to it (typically gitignored). `.cursor/rules/*.mdc` is a recognized file (CWD only) but is not part of the chain. `SOUL.md` is global identity, not project instructions |
...
- **Hermes** picks **one project context type** by first match (`.hermes.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`; `.cursor/rules/*.mdc` is recognized but sits outside the documented priority chain) — but when that type is `AGENTS.md` and cwd is inside a git repo it now loads a **merged ancestor chain**: the git-root `AGENTS.md` first, then every intermediate directory down to cwd, each with a provenance header (`## ../../AGENTS.md`) and identical copies deduplicated, deeper files later so they take precedence. Outside a git repo only cwd is read, so a stray `AGENTS.md` in `$HOME` or `/tmp` cannot leak in. It also does on-demand discovery of the dir + 5 parents (each checked at most once per session) during file ops.
...
- `permission_mode` is one of `default`, `plan`, `acceptEdits`, `auto`, `dontAsk`, `bypassPermissions` on Claude Code; Codex documents `default`, `acceptEdits`, `plan`, `dontAsk`, or `bypassPermissions` (no `auto`).
- Both engines share this schema, except that `prompt_id` is not a documented Codex field. Codex additionally provides `turn_id` and `model`.
...
- **Handler kinds** — `type: "command"` and `type: "mcp_tool"` run (an `mcp_tool` hook calls a tool on an already-connected MCP server, runs synchronously, and is not supported for `SessionEnd`); `prompt` and `agent` handlers are parsed but skipped. `async: true` **does** run a command hook in the background now (its own "Run hooks in the background" section on the Codex hooks page): background hooks keep the same input, matcher, trust review, timeout, and large-output handling as synchronous ones, and `SessionEnd` hooks always run synchronously even when `async` is set.
...
**Do not use `{"continue": false, "stopReason": "..."}` to block a single PreToolUse call** — on Claude Code `continue: false` is a universal field that stops Claude entirely ("Takes precedence over any event-specific decision fields"), and Codex marks `continue:false` on PreToolUse as unsupported; use `permissionDecision: "deny"` to block the call. Same applies to `{"continue": true}` as an allow signal. Mixing them up silently fails closed or open depending on the engine version.
...
**Matcher semantics: wildcards now agree, per-event honoring does not.** **Codex matchers are regex strings** (`^apply_patch$`, `Edit|Write`, `mcp__filesystem__.*`), but Codex special-cases the wildcard forms: `"*"`, `""`, or omitting `matcher` entirely matches every occurrence of a supported event — the same as Claude. So a Claude-style `"*"` matcher ports to Codex unchanged, and the old matcher-syntax divergence is gone. What still differs is **which events honor `matcher` at all**: on Codex, `UserPromptSubmit`, `Stop`, and `Interrupt` ignore any configured matcher, so scope those hooks inside the script rather than by matcher.

Codex tool coverage (verified 2026-08-19 against <https://learn.chatgpt.com/docs/hooks>): the hooks page now carries an explicit **Tool coverage** table, and it is wider than shell. `PreToolUse` and `PostToolUse` both fire for shell commands (match as `Bash`), unified exec `exec_command` (also matched as `Bash`), `apply_patch` (matched as `apply_patch`, `Edit`, or `Write`), MCP tools (`mcp__filesystem__read_file`), and **other local function tools** — matched by their function-tool name, such as `update_plan`, with `spawn_agent` also matching `Agent`. **Hosted tools such as `WebSearch` are the documented exception**: they do not use the local function-tool hook path, so no hook fires. A denying PreToolUse prevents the blocked `apply_patch` file from being created (`openai/codex#16732` fixed; PR `#18391`). The earlier "Bash only" behavior is obsolete — do **not** assume apply_patch is unhookable. Three things still bite, so design accordingly:

- **Coverage is a guardrail, not an enforcement boundary.** The docs state that "some specialized tool paths can opt out of the default hook path" and to "treat tool hooks as a useful guardrail, not a complete enforcement boundary", so a call can still slip past a `PreToolUse` guard. One opt-out is documented by name: `write_stdin` is transport for an existing unified-exec session, so it does **not** re-run `PreToolUse` when it sends input to or polls a command that already passed the gate — and a later `write_stdin` poll is what delivers the original command's `PostToolUse`.
