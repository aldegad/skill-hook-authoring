...
- **Cursor** — mixed: it ships **Cursor-owned** models (Composer 2.5 and Cursor Grok 4.6 / 4.5 from Cursor and SpaceXAI; Composer 1 no longer appears) *and* routes to third-party families (Anthropic through Claude Fable 5/Opus 5, OpenAI, Google, GLM 5.2, Kimi K2.7 Code/K3), with `Auto` as the default. Cursor Grok 4.6 supports `low`/`medium`/`high`/`xhigh` effort and a Fast tier. `Auto` is still not a pricing-table row and has three documented modes — Auto Cost, Auto Balance, Auto Intelligence — with Cursor Router picking the model on Teams/Enterprise. It is not a pure router.
