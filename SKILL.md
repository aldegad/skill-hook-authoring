...
| gajae-code (`gjc`, community) | `SKILL.md` bundled skills installed into the user `.gjc` directory by `gjc setup defaults` (exact path no longer stated; `gjc skills list`/`read <name>`); ships `deep-interview`, `ralplan`, `ultragoal`, `autoresearch` | not documented | not documented (README explicitly says it is "not a hidden plugin" for other CLIs) |
