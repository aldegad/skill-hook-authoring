...
| gajae-code (community) | `gjc` / `gjc --tmux` / `gjc --tmux --worktree <branch>` | not documented (the README dropped `--mode rpc`; external control is the broker-bound `gjc sdk session` CLI, or the Coordinator MCP bridge `gjc mcp-serve coordinator`) |
...
- **Claude / Claude Code** — current: Fable 5.1 (`claude-fable-5-1`, "for demanding reasoning and long-horizon agentic work" — supersedes Fable 5, 2026-09-11), Opus 5 (`claude-opus-5`, page now says "for most workloads"), Sonnet 5 (`claude-sonnet-5`), Haiku 4.5 (`claude-haiku-4-5`, id `claude-haiku-4-5-20251001`). Legacy: Fable 5 (→ 5.1, 2026-09-11), Opus 4.8 (→ 5), Opus 4.7, and Sonnet 4.6 (→ 5); `claude-opus-4-1-20250805` retired 2026-08-05 with Opus 5 as the stated migration target. Reasoning is **adaptive thinking** on the current generation (Haiku 4.5: extended thinking); the `--effort` labels (`low`, `medium`, `high`, `xhigh`, `max`, `ultracode`) are a Claude Code caller-layer selector, not distinct vendor models. Claude Mythos (Project Glasswing) is documented but invitation-only (not GA) — do not treat it as spawnable; its pricing footnote now references Mythos 5.1 (not re-verified against its own model page this run).
...
- **Cursor** — mixed: it ships **Cursor-owned** models (Composer 2.5 and Cursor Grok 4.6 / 4.5 from Cursor and SpaceXAI; Composer 1 no longer appears) *and* routes to a wider third-party lineup than before (2026-09-11: the Other Models table now spans the full Claude line up to Fable 5.1/Opus 5, the full GPT-5 line including GPT-5.1/5.2 Codex variants and GPT-5.4 Nano, Gemini 3 Pro/3.1 Pro/3.5–3.8 Flash, GLM 5.2, Kimi K2.7 Code/K3, and a new Meta entry `Muse Spark 1.3`), plus `Auto` (not available on the Start plan). Cursor Grok 4.6 supports `low`/`medium`/`high`/`xhigh` effort and a Fast tier, though the Start plan pins Grok 4.6 / 4.5 to a fixed medium effort in non-fast mode. `Auto` is still not a pricing-table row and has three documented modes — Cost, Balance, Intelligence, each billed at the routed model's list price — with Cursor Router picking the model on Teams/Enterprise.
