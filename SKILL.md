...
- **Gemini-Antigravity** — provider-routed; ids verified against Google's official model page (<https://ai.google.dev/gemini-api/docs/models?hl=en>, page last-updated 2026-09-04). **`gemini-3.8-flash` is the new Stable latest/most-capable Flash model** (2026-09-11 drift), engineered for long-horizon software engineering, autonomous agents, and complex enterprise workflows, superseding `gemini-3.7-flash` as the lead row; `gemini-3.7-flash` and `gemini-3.6-flash` remain Stable as previous generations, while `gemini-3.5-flash` is still labelled legacy. Stable `gemini-3.5-flash-lite`, `gemini-3.1-flash-lite`, and preview ids (`gemini-3.1-pro-preview`, `gemini-3-flash-preview`, Antigravity Agent `antigravity-preview-05-2026`) remain. The CLI's headless-mode sample (`https://antigravity.google/docs/cli/headless`, re-verified 2026-09-11) now surfaces `gemini-3.8-flash-high`, `gemini-3.8-flash-medium`, `gemini-3.7-flash-high`, `gemini-3.7-flash-medium`, `gemini-3.6-flash-high`, `gemini-3.6-flash-medium`, `gemini-3.1-pro-high`, and `claude-sonnet-4-6` — CLI-surface slugs not on the API models page; the `gemini-3.8-flash-*` pair is new this run and `gemini-3.5-flash-medium` no longer appears in the sample.
...
- **Cursor** — mixed: it ships **Cursor-owned** models (Composer 2.5 and Cursor Grok 4.6 / 4.5 from Cursor and SpaceXAI; Composer 1 no longer appears) *and* routes to a wider third-party lineup than before (2026-09-11: the Other Models table now spans the full Claude line up to Fable 5.1/Opus 5, the full GPT-5 line including GPT-5.1/5.2 Codex variants and GPT-5.4 Nano, Gemini 3 Pro/3.1 Pro/3.5–3.8 Flash, GLM 5.2, Kimi K2.7 Code/K3, and a new Meta entry `Muse Spark 1.3`), with `Auto` as the default. Cursor Grok 4.6 supports `low`/`medium`/`high`/`xhigh` effort and a Fast tier, though the Start plan pins Grok 4.6 / 4.5 to a fixed medium effort in non-fast mode. `Auto` is still not a pricing-table row and has three documented modes — Cost, Balance, Intelligence, each billed at the routed model's list price — with Cursor Router picking the model on Teams/Enterprise.
