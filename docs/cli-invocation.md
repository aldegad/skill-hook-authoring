...
its rows cite the project's GitHub README (verified 2026-09-18), not vendor docs.
...
|---|---|---|
| Codex | `codex` — "launches the interactive terminal UI (TUI)" | `codex "<prompt>"` — the CLI reference types the interactive positional as `PROMPT: string` ("Optional text instruction to start the session. Omit to launch the TUI without a pre-filled message."), so **no stdin `-` form is documented here**; the `PROMPT: string \| - (read stdin)` typing is documented only on `codex exec`, `codex exec resume`, and `codex review` |
...
| gajae-code (community) | `gjc` — interactive TUI; `gjc --tmux` runs it inside tmux, `gjc --tmux --worktree <name>` adds a git-worktree per task — the README notes "`--worktree` names a GJC-managed sibling worktree; for an existing path, `cd` there first." | `gjc @screenshot.png "<prompt>"` (image input); interactive clipboard image paste is not documented in the README. Install is a tagged prebuilt binary (`scripts/install.sh` / `install.ps1`, `--channel nightly` optional) — the earlier `bun install -g gajae-code` package install is no longer documented; Bun is only for building from source |

## B. Headless run (non-interactive / print / one-shot)

| Runtime | Headless command | Prompt / stdin | Output format | Useful scripting flags |
|---|---|---|---|---|
| Codex | `codex exec "<prompt>"` (alias `codex e`) | prompt arg; `-` reads prompt from stdin | `--json` / `--experimental-json` (JSONL); else final message to stdout, progress to stderr | `--model`/`-m`, `--output-last-message`/`-o <file>`, `--sandbox`/`-s`, `--cd`/`-C`, `--output-schema <file>` (JSON-Schema-constrained final message), `--ephemeral` (don't persist rollout files), `--ignore-user-config`, `--ignore-rules`, `--skip-git-repo-check` ("Allow running outside a Git repository (useful for one-off directories)"), `--full-auto` (deprecated — the docs say prefer `--sandbox workspace-write`); `CODEX_API_KEY` for inline auth. (Aside: `-p` is `--profile`, a config-profile selector — **not** a print/headless flag.) |
| Claude Code | `claude -p "<query>"` (`--print`) | `cat file \| claude -p "<query>"` | `--output-format text\|json\|stream-json` (+ `--input-format`) | `--model`, `--bare` (skip auto-discovery; recommended for scripts, will become default for `-p`), `--allowedTools`, `--permission-mode default\|manual\|acceptEdits\|plan\|auto\|dontAsk\|bypassPermissions` (`manual` = alias of `default`, v2.1.200), `--max-turns`, `--max-budget-usd`, `--json-schema`, `--system-prompt[-file]`, `--append-system-prompt[-file]`, `--permission-prompt-tool`, `--no-session-persistence`, `--plugin-url <url>` (fetch plugin zip for session), `--agents <json>` (inline subagent definitions), `--settings <file-or-json>`, `--effort low\|medium\|high\|xhigh\|max\|ultracode`, `--advisor <model>` ("a model alias, `fable`, `opus`, or `sonnet`, or a full model ID"; `fable` requires Fable access). **Not every flag combines with `-p`:** `--bg` is rejected ("Claude Code rejects `--bg`"; the CLI reference's `--bg` row reads "Can't be combined with `-p`/`--print`"), and `--cloud` with a task description is rejected too — `--cloud` with a *session ID* plus `-p` queues a message into that cloud session and exits. Also note `--bare` "never reads OAuth credentials or the system keychain", so a bare run needs `ANTHROPIC_API_KEY` (or an `apiKeyHelper`) and bills as API usage, not subscription |
...
> no separate per-run credit. As of 2026-09-18, the Agent SDK billing
...
> verified 2026-09-18; the `ANTHROPIC_API_KEY` caveat re-confirmed 2026-09-18 on
...
|---|---|---|---|
| Codex | `codex resume` (picker) | `codex resume <SESSION_ID>` — resume/archive/unarchive/delete accept a session ID (UUID) **or a session name** (`codex fork`'s `SESSION_ID` is typed `uuid` only), and "Session IDs take precedence over session names"; `--force` is documented as "Delete without prompting. The session argument must be a UUID; names still require interactive confirmation." | `codex exec resume --last` / `codex exec resume <SESSION_ID>` (`--all` widens past cwd) |
| Claude Code | `claude -c` (latest in cwd; interactive `-c` skips `-p`/Agent SDK sessions and sessions whose first prompt was `/loop`) | `claude -r "<id-or-name>"` (set a name with `-n`) | add `-p`: `claude -c -p` / `claude -r "<s>" -p` — unlike interactive `-c`, "`claude -p --continue` includes `-p`, SDK, and `/loop` sessions" |
| Grok / xAI | `grok -c` | `grok -r [<ID>]` (reference: omitting the id resumes the most recent) / `grok -s <ID>` (named) — the reference documents this as `-s, --session-id <UUID>` ("Use a specific UUID for a new session"); the headless-scripting page still shows the looser `<ID>`, so the two pages differ | add `-p` to the same flags |
| Hermes Agent | `hermes -c` / `hermes --resume latest` (`--in <dir>` scopes to that workspace) | `hermes -r <session_id>` (or by title; `hermes -c "<name>"` resumes the latest in that lineage) | not separately documented |
...
transcript for that ID, so a hand-copied duplicate reports not-found). As of
2026-09-11, `-r`/`--resume` also accepts, in place of a session ID, "the
absolute path to a session's `.jsonl` transcript file," and Claude Code
continues the conversation stored in that file. Also as of Claude Code
v2.1.257, `--continue` "opens a background session that has finished, but not
one that is still running" — a still-running background session is not a
`--continue` target. `claude --from-pr` is **not** a pin: it
...
- **Successor:** Antigravity CLI (`agy`) shares the agent harness with the
  ...
  aliases) and `--skip-path` (skip appending to PATH). Default auth is via the OS keyring (browser/SSH OAuth on first sign-in); a documented `GEMINI_API_KEY` + `modelProvider: "gemini"` path exists for headless/CI runs (see below).
...
- **`GEMINI_API_KEY` auth is now officially documented (2026-09-11).** The
  install page's "Using a Gemini API key" section states: "Run Antigravity CLI
  with your own Gemini API key instead of a signed-in Google account" for
  "headless and CI runs, where no browser is available to complete a sign-in."
  It requires **both** steps — set `modelProvider` to `gemini` in
  `~/.gemini/antigravity-cli/settings.json` **and** export `GEMINI_API_KEY`
  ("Only setting a `GEMINI_API_KEY` environment variable on its own has no
  effect"). `GOOGLE_GEMINI_BASE_URL` points at a custom endpoint. This retires
  the earlier "Auth is via the OS keyring, not an env var" verdict — keyring
  sign-in remains the default, but a documented API-key path now exists
  alongside it for unattended runs. No `AV_API_KEY` swap is documented anywhere
  (that third-party claim stays unconfirmed). (Source:
...
- **Source availability — Grok Build is the one tracked vendor CLI whose source
  ...
  check of `docs.x.ai/llms-full.txt` returns zero hits for "open source"/"Apache"/"MIT"),
...
- gajae-code (community project, README — not vendor docs; re-verified 2026-09-18) — <https://github.com/Yeachan-Heo/gajae-code>
...
- Claude Agent SDK / headless plan usage (2026-06-15 change **paused**, still paused as of 2026-09-18) — <https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan>
