...
| Hermes Agent | Official docs describe skills, skill taps, skills preloading, and plugin-bundled skills via `ctx.register_skill()`. | Official docs describe gateway hooks (`~/.hermes/hooks/`), shell hooks (`hooks:` in `~/.hermes/config.yaml`), and plugin hooks (`ctx.register_hook()`). | Official docs describe `plugin.yaml` plugins under `~/.hermes/plugins/` that can register tools, hooks, slash commands, CLI commands, and skills. | Supported project context files are `.hermes.md` / `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, and `.cursor/rules/*.mdc`; only one project context type loads per session by priority. `SOUL.md` is global identity from `HERMES_HOME`, not project context. | Official overview describes scheduled tasks; exact cron/update semantics must be verified from a deeper source before implementation claims. |
| Cursor CLI | Official docs describe Agent Skills from project `.agents/skills/` and `.cursor/skills/`, user `~/.agents/skills/` and `~/.cursor/skills/`, plus Claude/Codex compatibility skill roots. | Official hooks support project `.cursor/hooks.json`, cloud-agent project hooks, team/enterprise hooks, and command/prompt/file/agent lifecycle events. | Cursor plugin docs exist; hook `workspaceOpen` can return additional plugin paths, but Codex-style package parity is not documented here. | Official docs confirm `.cursor/rules`, project-root `AGENTS.md`, and project-root `CLAUDE.md` support. Worktree support via `--worktree` flag is also documented. | External scheduler or CI unless official Cursor automation docs are added. |
...
| Antigravity CLI | Yes: workspace `.agents/skills/`. | Partial: `/hooks` is documented in the TUI, but the config file format is not verified in the cited source set. | Treat hook packaging as unknown until the config source is verified. |
...
| Cursor CLI | Yes: project `.agents/skills/` and `.cursor/skills/`. | Yes: project `.cursor/hooks.json`. | Cloud agents also load repository hooks. |
