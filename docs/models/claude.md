...
Last reviewed: 2026-09-15 (verified live against the official models overview)
...
- The current generation uses **adaptive thinking** (Fable 5.1: always on); Haiku
  ...
  named options `low`, `medium`, `high`, `xhigh`, `max`, `ultracode` but states
  **no default** — the `high` defaults above are the models page's, scoped by its
  own footnote to "The effort parameter's default on the Claude API".
