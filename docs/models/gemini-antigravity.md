...
Last reviewed: 2026-09-11 (verified live against the official Google Gemini model
...
| Model | Status | Notes |
|---|---|---|
| `gemini-3.8-flash` | **Stable (new, 2026-09-11)** | New latest and most capable Flash model — "engineered for long-horizon software engineering, autonomous agents, and complex enterprise workflows." Leads the table, superseding `gemini-3.7-flash`. |
| `gemini-3.7-flash` | Stable | Now previous-generation (was the lead Flash model as of 2026-09-10). |
...
- Not re-checked this run: whether `agy models` gained a `gemini-3.8-flash-*`
  slug to match. The Antigravity CLI's own `agy models` sample output
  (headless-mode page, as last captured 2026-09-10) leads with
  `gemini-3.7-flash-high` and `gemini-3.7-flash-medium`, followed by
