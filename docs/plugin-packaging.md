...
Official Codex plugins have `.codex-plugin/plugin.json` at the plugin root. A plugin can point to bundled `skills`, `mcpServers`, `apps`, and `hooks` (the `hooks` field accepts a single path, an array of paths, an inline hooks object, or an array of inline hooks objects; the default hook file is `hooks/hooks.json`, and `${PLUGIN_ROOT}` substitution in hook commands is documented but `unverified this run`). Plugin hook commands receive the Codex-specific environment variables `PLUGIN_ROOT` (installed plugin root) and `PLUGIN_DATA` (writable data directory); Codex also sets `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` "for compatibility with existing plugin hooks". Plugin hooks are not automatically trusted; users must review and trust hook definitions. Marketplace files are read from repo `$REPO_ROOT/.agents/plugins/marketplace.json` and personal `~/.agents/plugins/marketplace.json` (a `.claude-plugin/marketplace.json` marketplace location is `not documented`); `codex plugin marketplace add/list/upgrade/remove` manages tracked sources, and installs land in `~/.codex/plugins/cache/<marketplace>/<plugin>/<version>/` (`local` as the version for local plugins).
...
Hermes hooks are split across three documented systems: gateway hooks under `~/.hermes/hooks/<name>/` (each a directory containing `HOOK.yaml` and `handler.py`), shell hooks declared in `hooks:` inside `~/.hermes/config.yaml`, and plugin hooks registered with `ctx.register_hook()`. Plugin hooks are categorized on a per-invocation-vs-per-turn axis: **control-flow hooks fire per tool invocation** (`pre_tool_call`, `post_tool_call`) and **lifecycle hooks fire once per turn, before/after the tool-calling loop** (`pre_llm_call`, `post_llm_call`). Also documented: rewriting hooks `transform_tool_result`, `transform_terminal_output`, `transform_llm_output`; `pre_verify`, which fires once per turn when the agent edited code, just before it verifies/finishes (related config `agent.verify_guidance: false`); session and subagent observers `on_session_start`, `on_session_end`, `on_session_finalize`, `on_session_reset`, `subagent_start`, `subagent_stop`; kanban events `kanban_task_claimed`, `kanban_task_completed`, `kanban_task_blocked`; and gateway-specific hooks `pre_gateway_dispatch`, `pre_approval_request`, `post_approval_response`. Caveat: the vendor's own two pages disagree on the axis — the plugins page groups `pre_*`/`transform_*` as control-flow against `post_*` as lifecycle, while the developer guide uses the per-invocation-vs-per-turn split adopted here; verify against the target Hermes version before relying on the grouping. Shell hooks require an approval dialog the first time each `(event, command)` pair runs; decisions persist to `~/.hermes/shell-hooks-allowlist.json`. Project-local hooks have a documented path: a plugin under `./.hermes/plugins/` (with `HERMES_ENABLE_PROJECT_PLUGINS=true`) registers them via `ctx.register_hook()`. In the cited sources, Hermes still does not document a repo-walked project-local skill root equivalent to `.agents/skills/`; for one-project custom skills, use a plugin or explicit config entry and document the install path.
