...
The official Claude Code docs (now at code.claude.com as of 2026-05-27) cover hooks, settings, subagents, and a skills system where custom commands are merged into skills (`.claude/commands/` and `.claude/skills/`), following the agentskills.io open standard. Plugin marketplace settings are now documented in official settings sources (`blockedMarketplaces`, `skillOverrides`, `allowedChannelPlugins`, `strictPluginOnlyCustomization`, `pluginTrustMessage`). (`enabledPlugins` is no longer in official docs; the per-plugin allowlist setting is now `allowedChannelPlugins`. Source: https://code.claude.com/docs/en/settings, verified 2026-06-02.) A dedicated plugin package format equivalent to Codex's `.codex-plugin/plugin.json` is not established in the cited sources, but skills and hooks can be configured through settings.json and skill frontmatter.
